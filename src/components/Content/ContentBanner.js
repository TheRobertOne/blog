...
import {
  action_content_banner_edit,
  action_content_banner_submit,
  action_content_banner_del,
  action_content_banner_delConfirm,
  action_content_banner_edit_clearAll,
  

} from "../../actions/content/contentBannerAction";
const Option = Select.Option;

class ContentBanner extends React.Component{
    componentDidMount(){
        console.log("进入 ContentBanner 组件 ",this.props); 
        this.props.getList();
    ...
    render(){
        ...
        return(<Spin spinning={listData.spinning}>
            <Button 
                ...
                onClick={e => {
                    const ul_banner =  document.getElementById("ul_banner").children;
                    let arr = [];
                    for(let i = 0;i<ul_banner.length;i++){
                      //left.push(ul_app[i].style.left);
                      arr.push({
                        "id":ul_banner[i].dataset.id,
                        "deleteFlag":ul_banner[i].dataset.flag,
                        "seq":getPos(ul_banner[i]).top*10000
                      });
                    }	
            ...
            >SUBMIT</Button>
                ...
                style={{margin:"15px 0 0 15px"}}
            >
				<Link to="/ContentMenu/ContentBannerAdd">添加Banner</Link>

            </Button>
            
            <ul id="ul_banner">
              {
                listData.bannerList.map((item,index) => {
...
const mapStateToProps = state => {
  return {
    listData:state.content.contentBanner
  }
}

const mapDispatchToProps = dispatch => {
  return {
	  getList:(...args) => dispatch(action_content_banner_edit(...args)),
	  onSubmit:(...args) => dispatch(action_content_banner_submit(...args)),
	  onDelete:(...args) => dispatch(action_content_banner_del(...args)),
	  onConfirm:(...args) => dispatch(action_content_banner_delConfirm(...args)),
      onClear:(...args) => dispatch(action_content_banner_edit_clearAll(...args))
...
function drag_yf(){
        ...
        var aLi=document.getElementById('ul_banner').children;
